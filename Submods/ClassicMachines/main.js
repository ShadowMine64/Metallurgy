/*Metallurgy Classic Machines port by ShadowMine
 //Version 1.2
 //Changelog - 
 //	Rewritten
 //	Cleaner
 //	Block rotations
 // 	Metallurgy 4 now a dependency!
 */
/* BetterStorage API made by DAW330073 */
...
// metals = inform.scripts().find("Metallurgy4[unminified].js").get("metals");
...
JSON.readFile = function(directory){
	var file = ModPE.readFile(directory);
	var json_form = JSON.parse(file);
	return json_form;
};
...
function downloadMetalInformation(){//Because inform is broken
	if(!downloaded){
		var runnable = new java.lang.Runnable(){
			run: function(){
				try {
					var url = new java.net.URL();//Where all the info is stored
					var connection = url.openConnection();//Establish connection to github link
					connection.setRequestMethod('GET');//Get from the link
					connection.setDoOutput(true);
					connection.connect();//Connect to the github link
					connection.getContentLength();
					
					var file = connection.getInputStream();
					var type = java.lang.Array.newInstance(java.lang.Byte.TYPE, 1024);
					var byte_count = 0;
					while((byte_count = file.read(type)) != -1){
						info_file+=new java.lang.String(type, 0, byte_count);
					}
					var modfolder = activity.getDir("mods", 0);
					var informfile = new java.io.File(modfolder, "Metallurgy.json");
					var download = new java.io.PrintWriter(informfile);
					download.write(info_file);
					download.flush();
					download.close();
				}
				catch(err){
					print("Failed to download metal info");
				}
			}
		}
	}
...
function setMetals(){
	try{
		metals = JSON.parse("/games/com.mojang/minecraftpe/mods/Metallurgy.json");
	}
	catch(err){
		throw new Error("Can't enable mod");
	}
};

var mod = {};
mod.metallurgy_cm = {};
mod.metallurgy_cm.data = {};

mod.metallurgy_cm.data.ore_data = {
	iron: {
		block_id: 15,
		rssId: 2940
	},
...
mod.metallurgy_cm.data.blocks = {
	brass_chest: {
		id: 200,
		name: "Brass Chest",
		slots: 54,
		recipe: [["bbb", "bcb", "bbb"], ["b", metals[11].rssId[0], 0, "c", 54, 0]]
	},
	electrum_chest: {
		id: 201,
		name: "Electrum Chest",
		slots: 90,
		recipe: [["bbb", "bcb", "bbb"], ["b", metals[12].rssId[0], 0, "c", 202, 0]]
	},
	gold_chest: {
		id: 202,
		name: "Gold Chest",
		slots: 81,
		recipe: [["bbb", "bcb", "bbb"], ["b", metals.vanilla.gold.rssId[0], 0, "c", 204, 0]]
	},
	platinum_chest: {
		id: 203,
		name: "Platinum Chest",
		slots: 108,
		recipe: [["bbb", "bcb", "bbb"], ["b", metals.precious.platinum.rssId[0], 0, "c", 201, 0]]
	},
	silver_chest: {
		id: 204,
		name: "Silver Chest",
		slots: 72,
		recipe: [["bbb", "bcb", "bbb"], ["b", metals.precious.silver.rssId[0], 0, "c", 200, 0]]
	}
};

//For laziness
var Blocks = mod.metallurgy_cm.data.blocks;

//Define the blocks
Inventory.defineBlock(Blocks.brass_chest.id, [
...
], 1, false, 0, Blocks.brass_chest.slots);

/*
 //Custom hooks
 Item.isCrushableOre = function(item_id){
 for(var i in mod.metallurgy_cm.data.ore_data){
 if(item_id == mod.metallurgy_cm.data.ore_data[i].block_id){
 return true;
 } else {
 return false;
 }
 ...
 Item.returnOreInfo = function(item_id){
 for(var i in mod.metallurgy_cm.data.ore_data){
 if(mod.metallurgy_cm.data.ore_data[i].block_id == item_id){
 return mod.metallurgy_cm.data.ore_data[i];
 }
 }
 };

 Item.isMetal = function(item_id){
 for(var i in metals){
 for(var ii in metals[i]){
 if(metals[i][ii].type != "Drop"){
 if(item_id == metals[i][ii].rssId[0]){
 return true;
 }
 }
 }
 ...
 Item.returnMetalInfo = function(item_id){
 for(var i in metals){
 for(var ii in metals[i]){
 if(metals[i][ii].type != "Drop"){
 if(metals[i][ii].rssId[0] == item_id){
 return metals[i][ii];
 }
 }
 }
...
mod.metallurgy_cm.newLevel = function(){

};

mod.metallurgy_cm.modTick = function(){

};

mod.metallurgy_cm.useItem = function(x, y, z, item_id, block_id, side, item_data, block_data){
	switch(item_id){
...
mod.metallurgy_cm.destroyBlock = function(x, y, z){

};

mod.metallurgy_cm.leaveGame = function(){

};

//ModPE hooks
function newLevel(){BetterStorage.newLevel();mod.metallurgy_cm.newLevel();}
function modTick(){BetterStorage.modTick();mod.metallurgy_cm.modTick();}
function useItem(x,y,z,itemID,blockID,side,itemDat,blockDat){BetterStorage.useItem(x,y,z,itemID,blockID,side,itemDat,blockDat);mod.metallurgy_cm.useItem(x, y, z, itemID, blockID, side, itemDat, blockDat);}
function destroyBlock(x,y,z,side){BetterStorage.destroyBlock(x,y,z,side);mod.metallurgy_cm.destroyBlock(x, y, z);}
	...
	activity.runOnUiThread(new java.lang.Runnable({
													  run: function(){
														  try{
															  ...
															  rightTopBarText.setText("Metallurgy");
...
function DefineInventory(id,name,texture,materialSourceId,opaque,rendertype,size){
	...
	Block.setShape(id, 1/16, 0, 1/16, 15/16, 15/16, 15/16);
